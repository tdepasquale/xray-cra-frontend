...
export const PostsToApprove = () => {
  ...
  const getBlogs = async () => {
    try {
      const blogsFromDB = await agent.BlogPosts.listSubmittedBlogs();
  ...
  const rejectBlog = async () => {
    ...
    try {
      ...
      toast.error("Error rejecting blog post.");
    }
  };
  ...
  const ApproveModal = () => {
    return (
      <Modal
        ...
        size="tiny"
      >
        <Modal.Header>Approve Blog Post</Modal.Header>
        <Modal.Content>
          <Modal.Description>
  ...
  const NoBlogSegment = () => {
    return (
      <Segment placeholder>
        <Header icon>
          <Icon name="bell" />
          There don't seem to be any blog posts to approve.
        </Header>
      </Segment>
    );
  };
    ...
    return (
      <Segment>
        <Breadcrumb className="padding-bottom-1">
          ...
          <Breadcrumb.Section>Blog Posts to Approve</Breadcrumb.Section>
        </Breadcrumb>
      </Segment>
    );
  };
  ...
  return (
    <div className="chart-background">
      <Helmet>
        <title>{`Blog Posts to Approve | Rad X-Rays`}</title>
      </Helmet>
