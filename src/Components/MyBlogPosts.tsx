...
export const MyBlogPosts = () => {
  ...
  const getBlogs = async () => {
    try {
      const blogsFromDB = await agent.BlogPosts.listMyBlogs();
  ...
  if (pageIsLoading) return <Loading content="Loading blog posts..." />;

  const addBlog = async () => {
    setAddIsLoading(true);
    try {
      const response = await agent.BlogPosts.createBlogPost();
      setBlogPosts([response, ...blogPosts!]);
      setAddIsLoading(false);
      toast.success("A new blog post was created successfully.");
    } catch (error) {
  ...
  const SubmitModal = () => {
    return (
      <Modal
        ...
        size="tiny"
      >
        <Modal.Header>Submit Blog Post</Modal.Header>
        <Modal.Content>
          <Modal.Description>
    ...
    return (
      <Segment>
        <Breadcrumb className="padding-bottom-1">
          ...
          <Breadcrumb.Section>My Blog Posts</Breadcrumb.Section>
        </Breadcrumb>
      </Segment>
    );
  };
  ...
  const NoBlogSegment = () => {
    return (
      <Segment placeholder>
        <Header icon>
          <Icon name="bell" />
          You don't seem to have any unsubmitted blog posts.
        </Header>
  ...
  return (
    <div className="chart-background">
      <Helmet>
        <title>{`My Blog Posts | Rad X-Rays`}</title>
      </Helmet>
